...
const processDecelerationRate = function(decelerationRate) {
...
const { RNCUIWebViewManager } = NativeModules;
const { RNCWKWebViewManager } = NativeModules;
...
const WebView = /** @class */ (function(_super) {
  ...
  function WebView() {
    ...
    _this.injectJavaScript = function(data) {
      UIManager.dispatchViewManagerCommand(
        _this.getWebViewHandle(),
        _this.getCommands().injectJavaScript,
        [data],
      );
    };

    _this.isJump = function(data) {
      UIManager.dispatchViewManagerCommand(
        _this.getWebViewHandle(),
        _this.getCommands().isJump,
        [data],
      );
    };
    /**
    ...
    _this.onShouldStartLoadWithRequestCallback = function(
      shouldStart,
      _url,
      lockIdentifier,
    ) {
      let { viewManager } = _this.props.nativeConfig || {};
  ...
  WebView.prototype.render = function() {
    const _a = this.props;
    const decelerationRateProp = _a.decelerationRate;
    const _b = _a.nativeConfig;
    const nativeConfig = _b === void 0 ? {} : _b;
    const { onMessage } = _a;
    const onShouldStartLoadWithRequestProp = _a.onShouldStartLoadWithRequest;
    const { originWhitelist } = _a;
    const { renderError } = _a;
    const { renderLoading } = _a;
    const _c = _a.scalesPageToFit;
    const scalesPageToFit
      = _c === void 0 ? (this.props.useWebKit ? undefined : true) : _c;
    const { style } = _a;
    const { useWebKit } = _a;
